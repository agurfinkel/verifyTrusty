/** Verify Trusty
    ...
    Implementation of TrustyOS Messaging API with wide memory manager
...
extern size_t nd_msg_len(void);
extern uint32_t nd_msg_id(void);
extern int nd_get_msg_ret(void);
...
//#define SEA_PRECONDITION(X) assume(X)
#define SEA_PRECONDITION(X)

extern ssize_t nd_read_msg_ret(void);
extern uint8_t nd_msg_element(void);
...
ssize_t read_msg(handle_t handle, uint32_t msg_id, uint32_t offset,
                 ipc_msg_t *msg) {
  SEA_PRECONDITION(!msg_retired(msg_id));
...
extern ssize_t nd_send_msg_ret(void);
...
extern int nd_put_msg_ret(void);
...
int put_msg(handle_t handle, uint32_t msg_id) {
  ...
  (void)handle;
  cur_msg_retired = (cur_msg_id == msg_id);
  return nd_put_msg_ret();
}

extern int nd_wait_ret(void);
extern uint32_t nd_event_flag(void);
...
extern handle_t nd_wait_handle(void);
extern int nd_wait_any_ret(void);
...
extern handle_t nd_port_handle(void);
...
extern int nd_set_cookie_ret(void);
...
extern handle_t nd_chan_handle(void);
extern uint32_t nd_time_low(void);
extern uint16_t nd_time_mid(void);
extern uint16_t nd_time_hi_n_ver(void);
